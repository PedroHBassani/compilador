...
from parser_ply import parser, syntax_errors
...
token_specification = [
    ...
    ('AND',           r'&&'),
    ('OR',            r'\|\|'),
    ('EQUALS',        r'=='),  # <-- colocado antes de ASSIGN
    ('NOT_EQUALS',    r'!='),
    ('LESS_THAN_EQ',  r'<='), 
    ('GREATER_THAN_EQ', r'>='),
    ('ASSIGN',        r'='),  # <-- depois de EQUALS
    ('NOT',           r'!'),
    ('LESS_THAN',     r'<'),
    ('GREATER_THAN',  r'>'),
    ('PLUS',          r'\+'),
...
def run_analysis():
    ...
    class TokenWrapper:
        ...
        def token(self):
            if self.index < len(self.tokens):
                ...
                self.index += 1
                # Cria um objeto compatível com PLY
    ...
    if syntax_errors:
        for err in syntax_errors:
            errors_output.insert(tk.END, f"{err}\n")
    else:
